function hogfeatEncoding(dataset_path,ClusterNum)


load(['./data/codebooks/hogfeat_kmeans.mat'], 'centers');
centers = centers';
...
dtdir = [dataset_path 'hogfeat'];    % ****
dtlist = dir(fullfile(dtdir, '*.mat'));
mkdir('./data/encoding');
mkdir('./data/encoding/hogfeat');   % ****
matlabpool(30)
parfor id = 1 : length(dtlist)
    
    hog_temp = load(fullfile(dtdir, dtlist(id).name), 'hogfeat');% ****
    hogfeat = hog_temp.hogfeat;
    
    [idx , dist] = knnsearch(centers, hogfeat,'k',1);
...
%     load(['./encoding/dt/', dtlist(id).name], 'idx');
    ...
    parsave(['./data/encoding/hogfeat/', dtlist(id).name], idx, enDtFeat);   % ****
end

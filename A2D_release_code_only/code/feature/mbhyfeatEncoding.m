function mbhyfeatEncoding(dataset_path,ClusterNum)


load(['./data/codebooks/mbhyfeat_kmeans.mat'], 'centers');% ****
centers = centers';
...
dtdir = [dataset_path 'mbhyfeat'];    % ****
dtlist = dir(fullfile(dtdir, '*.mat'));
mkdir('./data/encoding');
mkdir('./data/encoding/mbhyfeat');   % ****
matlabpool(30);
parfor id = 1 : length(dtlist)
    
    mbhy_temp = load(fullfile(dtdir, dtlist(id).name), 'mbhyfeat');% ****
    mbhyfeat = mbhy_temp.mbhyfeat;
    
    [idx , dist] = knnsearch(centers, mbhyfeat,'k',1);
...
%     load(['./encoding/dt/', dtlist(id).name], 'idx');
    ...
    parsave(['./data/encoding/mbhyfeat/', dtlist(id).name], idx, enDtFeat);   % ****
end
matlabpool close;

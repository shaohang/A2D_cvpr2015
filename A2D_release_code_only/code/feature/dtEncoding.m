function dtEncoding(dataset_path,ClusterNum)


load(['./data/codebooks/dtfeat_kmeans.mat'], 'centers');
centers = centers';
...
dtdir = [dataset_path 'dtfeat'];
dtlist = dir(fullfile(dtdir, '*.mat'));
mkdir('./data/encoding');
mkdir('./data/encoding/dtfeat');  

matlabpool(30)
parfor id = 1 : length(dtlist) 
    dtfeat_temp = load(fullfile(dtdir, dtlist(id).name), 'dtfeat');
    dtfeat = dtfeat_temp.dtfeat;
    [idx , dist] = knnsearch(centers, dtfeat,'k',1);
    
    dtinfo_temp = load(fullfile(infodir, infolist(id).name), 'dtinfo');
    StInfo = dtinfo_temp.dtinfo(:,8:10); % 8 col, 9 row, 10 time
%     load(['./encoding/dt/', dtlist(id).name], 'idx');
    ...
    parsave(['./data/encoding/dtfeat/', dtlist(id).name], idx, enDtFeat);

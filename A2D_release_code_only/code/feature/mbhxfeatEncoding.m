function mbhxfeatEncoding(dataset_path,ClusterNum)


load(['./data/codebooks/mbhxfeat_kmeans.mat'], 'centers');% ****
centers = centers';
...
dtdir = [dataset_path 'mbhxfeat'];    % ****
dtlist = dir(fullfile(dtdir, '*.mat'));
mkdir('./data/encoding');
mkdir('./data/encoding/mbhxfeat');   % ****
matlabpool(30);
parfor id = 1 : length(dtlist)
    
    mbhx_temp = load(fullfile(dtdir, dtlist(id).name), 'mbhxfeat');% ****
    mbhxfeat = mbhx_temp.mbhxfeat;

    [idx , dist] = knnsearch(centers, mbhxfeat,'k',1);
...
%     load(['./encoding/dt/', dtlist(id).name], 'idx');
    ...
    parsave(['./data/encoding/mbhxfeat/', dtlist(id).name], idx, enDtFeat);   % ****
end
matlabpool close;

...
function actor_naive_multilable(dataset_path,number_clusters)
...
% revise multi-label table
number_actor = 7;
actor_train_label = zeros(number_train, number_actor);
actor_test_label = zeros(number_test, number_actor);
for i = 1:number_actor
    temp_idx = find(floor(class_idx/10)==i);
    for j = 1:number_train
        if isempty(find(train_label(j,temp_idx)==1))~=1
            actor_train_label(j,i) = 1;
        end
    end
    
    for j = 1: number_test
        if isempty(find(test_label(j,temp_idx)==1))~=1
            actor_test_label(j,i) = 1;
        end
    end
end

train_label = actor_train_label;
test_label = actor_test_label;
...
actor_predict_label = zeros(size(test_label,1),number_active_class);
for i = 1:number_active_class
    ...
    actor_predict_label(:,i) = predict_label;
end
...
disp(['Actor Mean Average Precision(NB): ' num2str(mean_average_precision)]);
save('./data/actor_prob_ml.mat', 'prob', 'actor_predict_label');
